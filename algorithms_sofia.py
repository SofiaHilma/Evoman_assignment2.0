...
### Diverse algorithm
    ...
    def fitness_sharing(self, population, fitness, n_parents, sigma_share=0.7):
        ...
        # Return indices instead of population subset
        return selected_indices
    ...
    def deterministic_crowding(self, parents, offspring, parent_fitness, offspring_fitness):
        new_population_indices = []
        for i in range(0, len(parents), 2):
            p1, p2 = parents[i], parents[i+1]
            c1, c2 = offspring[i], offspring[i+1]

            d11 = np.linalg.norm(p1 - c1)
            d12 = np.linalg.norm(p1 - c2)
            d21 = np.linalg.norm(p2 - c1)
            d22 = np.linalg.norm(p2 - c2)

            if d11 + d22 <= d12 + d21:
                if offspring_fitness[i] > parent_fitness[i]:
                    new_population_indices.append(self.pop_size + i)  # Index of c1 in combined population
                else:
                    new_population_indices.append(i)  # Index of p1
                if offspring_fitness[i+1] > parent_fitness[i+1]:
                    new_population_indices.append(self.pop_size + i + 1)  # Index of c2
                else:
                    new_population_indices.append(i + 1)  # Index of p2
            else:
                if offspring_fitness[i+1] > parent_fitness[i]:
                    new_population_indices.append(self.pop_size + i + 1)  # Index of c2
                else:
                    new_population_indices.append(i)  # Index of p1
                if offspring_fitness[i] > parent_fitness[i+1]:
                    new_population_indices.append(self.pop_size + i)  # Index of c1
                else:
                    new_population_indices.append(i + 1)  # Index of p2

        return np.array(new_population_indices)

    def diverse_mutation(self, individual, mutation_rate, scale=0.7):
    ...
    def triggered_diverse_evolve(self, n_parents=10, mutation_rate=0.2,
        ...
        for generation in range(1, self.max_gens + 1):
            ...
            if counter == 3:
                ...
                parent_indices = self.fitness_sharing(self.population, fitness_values, n_parents=n_parents)
                ...
                mutated_offspring = np.array([self.diverse_mutation(child, mutation_rate=mutation_rate) for child in offspring])
                ...
                survivor_indices = self.deterministic_crowding(self.population, mutated_offspring, fitness_values, offspring_fitness)
                self.population = combined_population[survivor_indices]
                self.track_fitness(combined_fitness[survivor_indices])
                ...
                # Step 2: Select parents using the specified selection method
                parent_indices = self.roulette_wheel_selection(self.population, fitness_values, n_parents=n_parents)
                parents = self.population[parent_indices]
                ...
                survivor_indices = self.roulette_wheel_selection(combined_population, combined_fitness, n_parents=self.pop_size)
                self.population = combined_population[survivor_indices]
                self.track_fitness(combined_fitness[survivor_indices])
